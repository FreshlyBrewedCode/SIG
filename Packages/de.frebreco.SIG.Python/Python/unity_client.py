...
try:
    ...
    path = os.path.normpath(os.path.dirname(
        "D:/Dokumente/Bachelorarbeit/Image Generator/Library/PackageCache/com.unity.scripting.python@2.1.1-preview.1/Python~/site-packages/__init__.py"))
    sys.path = [path] + sys.path
...
def connect(Service=UnityClientService):
    ...
    """
    config = {'propagate_SystemExit_locally': True,
              'propagate_KeyboardInterrupt_locally': True}
    connection = rpyc.utils.factory.unix_connect(
        settings.unity_server_path, Service, config=config)
    connection_thread = threading.Thread(
        target=connection.serve_all, name="Connection to Unity")
